...
def create_batches(dataset, batch_size, pad_token_id, bidirectional_prefix=False):
    """
...
def train_model(
    ...
    epochs=3, 
    eval_every=100, 
    optimizer_params={"learning_rate": 1e-5, "min_lr_factor": 0.1, "warmup_steps": 20},
    ...
    train_batches = create_batches(train_set, batch_size, pad_token_id, bidirectional_prefix=bidirectional_prefix)
    NUM_TRAIN_EXAMPLES = len(train_set)
    val_batches = create_batches(val_set, batch_size, pad_token_id, bidirectional_prefix=bidirectional_prefix)
...
def evaluate_model(model_name, model_type, model_path, test_set, batch_size=8, spectral_dims=None, report_file=None, bidirectional_prefix=False):
    """
    ...
    test_batches = create_batches(test_set, batch_size, pad_token_id, bidirectional_prefix=bidirectional_prefix)
...
if __name__ == "__main__":
    ...
    train_model("graph_llama", MODEL_NAME, train_set, val_set, parameters, epochs=EPOCHS, batch_size=4, eval_every=50, bidirectional_prefix=BIDIRECTIONAL_PREFIX)
    ...
    # evaluate_model(MODEL_NAME, "graph_llama", "easy_graph_llama_model.pth", val_set, spectral_dims=parameters['spectral_dims'], report_file="easy_graph_llama_report.jsonl", bidirectional_prefix=BIDIRECTIONAL_PREFIX)
