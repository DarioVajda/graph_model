...
def build_parser():
    ...
    p.add_argument("--magnetic-linear-v2", action=B, default=d.magnetic_linear_v2,
                   help="Linear phase channel with its eigenvalue features scaled "
                        "by a bounded per-node gate read off the spectral "
                        "self-energy. Measured null on WebQSP. "
                        "Equals --magnetic-linear at init. Use with --no-magnetic.")
    p.add_argument("--magnetic-gate-repr-dim", type=int, default=d.magnetic_gate_repr_dim,
                   help="Gate-MLP hidden width for --magnetic-linear-v2. Internal, "
                        "so it never reaches attention and is free.")
    p.add_argument("--magnetic-nonlinear", action=B, default=d.magnetic_nonlinear,
                   help="Per-node features built by attention-pooling a whole row "
                        ...
                        "instead of its diagonal. See NON_LINEAR_BIAS.md. Use with "
                        "--no-magnetic.")
    p.add_argument("--magnetic-struct-dim", type=int, default=d.magnetic_struct_dim,
                   help="d_struct: width Q/K append per head for "
                        "--magnetic-nonlinear. NOT free.")
    p.add_argument("--magnetic-pool", choices=("attn", "uniform"), default=d.magnetic_pool,
                   help="Pooling for --magnetic-nonlinear: 'attn' is the learned "
                        "pool, 'uniform' the 1/n_b ablation.")
