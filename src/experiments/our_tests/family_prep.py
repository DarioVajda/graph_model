...
def prepare_graph(graph, person_id, question, answer, question_node="off"):
    # add texts to nodes
    ...
    # Split question/answer across nodes per `question_node` (see prompt.py). With "off"
    # this is exactly the historical single-node layout; with "isolated" the question
    # gets its own edge-free node and the prompt node keeps the "A:" anchor.
    prompt_node_id = f"prompt_{person_id}"
    question_text, prompt_text = node_texts(question, answer, question_node)

    if question_text is not None:
        question_node_id = f"question_{person_id}"
        incidence_graph.add_node(question_node_id, text=question_text)
        incidence_graph.graph['question_node'] = question_node_id

    incidence_graph.add_node(prompt_node_id, text=prompt_text)

    # add edge from the prompt node to the person node about whom the question is being
    # asked (the question node stays edge-free — only the attention mask connects it)
...
def prepare_graph_dataset(raw_dataset, question_node="off"):
    prepared_dataset = {}
...
# data_prep.py so the question_node feature is implemented once.
#endregion
