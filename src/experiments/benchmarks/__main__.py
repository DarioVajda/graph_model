...
def training_run(
    ...
    import time
    # use the current time to get a unique numerical seed for the run
    seed = int(time.time()) % (2**16 - 1)
    print(f"Using seed {seed} for this training run.")
    ...
    training_args = TrainingArguments(
        ...
        data_seed=seed,                                         # Random seed for data shuffling
    )
...
if __name__ == "__main__":
    ...
    RUN_NAME = f"{model_size}_{dataset_name}{'_lora' if LORA_CONFIG else ''}"
    ...
    datasets = load_dataset(dataset_dir, train=True, val=True, test=True, max_val_size=500)
