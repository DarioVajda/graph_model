...
error, not a warning.
    ...
    python3 -m src.experiments.nonlinear_bias.stratified_eval submit [--dry-run]
...
OUT_ROOT = os.path.join(REPO, "src/experiments/nonlinear_bias/results", SWEEP)
...
ARMS = [
    ("arm2-linear", "src/experiments/mixed_bias/results/021_webqsp_baselines",
     lambda r: "magnetic_linearTrue" in r and "bias_lr0.005" in r),
    ("N-uniform", "src/experiments/nonlinear_bias/results/032_webqsp_nonlinear",
     lambda r: "pooluniform" in r and "bias_lr0.005" in r),
    ("N-attn", "src/experiments/nonlinear_bias/results/032_webqsp_nonlinear",
...
def cmd_submit(a):
    ...
    def write_job(arm, sweep_id, run_name, job_script, ck_run, runs_jsonl,
        ...
        with open(sh, "w") as f:
            f.write("#!/usr/bin/env bash\nset -uo pipefail\n"
                    "python -m src.experiments.nonlinear_bias.stratified_eval run"
    ...
    print(f"  then:      python3 -m src.experiments.nonlinear_bias.stratified_eval report")
    return 0
