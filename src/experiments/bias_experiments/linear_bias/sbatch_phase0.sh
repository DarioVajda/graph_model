...
#
#   ./src/experiments/bias_experiments/linear_bias/sbatch_phase0.sh            # both sweeps, 3 seeds
#   ./src/experiments/bias_experiments/linear_bias/sbatch_phase0.sh smoke      # one seed, few graphs
#   GPU=H100 ./src/experiments/bias_experiments/linear_bias/sbatch_phase0.sh
# =============================================================================
set -euo pipefail
...
OUT_DIR="$REPO/src/experiments/bias_experiments/linear_bias/results"
...

{
  ...
  [ "$DO_WEBQSP" = 1 ] && for s in $SEEDS; do
    ...
    echo "python -m src.experiments.bias_experiments.linear_bias.phase0 \\"
    echo "  --job \$(ls src/experiments/bias_experiments/bias_sharing/results/002_webqsp_g_sweep/jobs/*_seed${s}_magnetic_groups0.sh) \\"
  ...
  [ "$DO_CONTEXT" = 1 ] && for s in $SEEDS; do
    echo "python -m src.experiments.bias_experiments.linear_bias.phase0 \\"
    echo "  --job \$(ls src/experiments/bias_experiments/bias_sharing/results/003_context4k_g_sweep/jobs/*_seed${s}_magnetic_groups0.sh) \\"
  ...
  echo "python -m src.experiments.bias_experiments.linear_bias.analyse --results $OUT_DIR"
} > "$SCRIPT"
chmod +x "$SCRIPT"
