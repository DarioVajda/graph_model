...
#
#   ./src/experiments/bias_experiments/bias_sharing/bench/sbatch_speed.sh                 # everything
#   ./src/experiments/bias_experiments/bias_sharing/bench/sbatch_speed.sh smoke           # 2 min sanity
#   GPU=B300 ./src/experiments/bias_experiments/bias_sharing/bench/sbatch_speed.sh
#
...
OUT_DIR="$REPO/src/experiments/bias_experiments/bias_sharing/results/bench"
...
LOG="$REPO/job_logs/bias_bench_${MODE}_${STAMP}_%j.out"
{
  ...
  if [ "$MODE" = "biasmodes" ]; then
    echo "python -m src.experiments.bias_experiments.bias_sharing.bench.bias_modes"
  else
    echo "python -m src.experiments.bias_experiments.bias_sharing.bench.speed $ARGS"
  fi
} > "$SCRIPT"
chmod +x "$SCRIPT"
