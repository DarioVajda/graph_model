...
SCRIPT="$REPO/job_logs/landmark_diag_${STAMP}.sh"
LOG="$REPO/job_logs/landmark_diag_${STAMP}_%j.out"
{ echo "#!/usr/bin/env bash"; echo "set -x"
  echo "python -m src.experiments.bias_experiments.landmark.diagnose_scale"
...
bash $REPO/sweep/slurm_launch.sh landmark_diag_${STAMP} $SCRIPT"
JOB=$(sbatch --parsable -p "$PARTITION" -A povejmo -c 8 --mem 32G -t "00:30:00" \
      -J "landmark_diag" -o "$LOG" --wrap "$WRAP" | grep -oE '^[0-9]+$' | head -1)
