...
SCRIPT="$REPO/job_logs/landmark_diagnostic_${STAMP}.sh"
LOG="$REPO/job_logs/landmark_diagnostic_${STAMP}_%j.out"

{
  echo "#!/usr/bin/env bash"
  echo "set -x"
  echo "python -m src.experiments.bias_experiments.landmark.anchor_diagnostic ${DIAG_ARGS:-}"
} > "$SCRIPT"
chmod +x "$SCRIPT"
...
bash $REPO/sweep/slurm_launch.sh landmark_diagnostic_${STAMP} $SCRIPT"
...
JOB=$(sbatch --parsable -p "$PARTITION" -A povejmo -c 16 --mem 64G -t "04:00:00" \
      -J "landmark_diagnostic" -o "$LOG" --wrap "$WRAP" \
      | grep -oE '^[0-9]+$' | head -1)
