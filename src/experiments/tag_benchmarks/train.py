"""
Train ONE TAG-benchmark configuration and log ONE record.

Protocol: train on the train split, evaluate + checkpoint every ``eval_steps``, reload
the best-**validation** checkpoint at the end (HF ``load_best_model_at_end``), and
report that checkpoint's **test** exact match.

This is a deliberate change from the pre-refactor loop, which set the same
best-val-checkpoint machinery up and then ignored it: it scored the three surviving
checkpoints on the TEST split and reported the best of them, i.e. it selected on test.
Numbers produced here are therefore expected to be LOWER than previously reported ones,
which were optimistically biased. Selection now touches val only, and test is scored
exactly once, on one checkpoint.

The model is the v2 GTLM stack (``GTLMLlamaForCausalLM``); see ``config.py`` on the v0
parity evidence and its one gap (the historical runs used v0 + sdpa).
"""
...
def _save_train_record(cfg, run_name, sizes, samples_per_node, results, runs_jsonl,
                       sweep_meta=None):
    ...
    record = {
        ...
        **(sweep_meta or {}),
        ...
        "dataset": cfg.dataset, "text_mapping": cfg.text_mapping,
        "text_mapping_param": cfg.text_mapping_param,
        "mapping_name": cfg.mapping_name(),
        "hops": cfg.hops, "max_neighbors": cfg.max_neighbors,
        ...
        # The augmentation the cache actually holds — not in the directory name, so
        # record what was loaded rather than what was asked for.
        "samples_per_node": samples_per_node,
        "train_size": sizes[0], "val_size": sizes[1], "test_size": sizes[2],
        "val_subsample": cfg.val_subsample, "test_subsample": cfg.test_subsample,
        # ── results ──
        **(results or {}),
    }
...
def run_train_mode(cfg, runs_jsonl=None, run_name=None, sweep_id=None):
    ...
    samples_per_node = train_dataset.per_graph_versions

    # Seed before the build so the graph-bias parameter init is determined by `seed` too,
    # not just the data order — this pairs ablation arms within a seed. (HF's Trainer
    # seeds itself, but only once the model already exists.) The pre-refactor loop used
    # `int(time.time()) % 65535`, so no run here was ever reproducible.
    set_seed(cfg.seed)
    ...
    training_args = TrainingArguments(
        ...
        # Match eval batch to train batch: flex compiles per (B, L, N) shape family, so
        # HF's default eval batch of 8 would mint a second kernel set.
        ...
        # 10% of total steps, as the pre-refactor schedule did.
    ...
    # The best-val checkpoint is loaded now: score it on val (sanity) and on test (the
    # reported metric). GraphTrainerV2.get_eval_dataloader invalidates HF's constant
    # "eval"-key dataloader cache when a different split is passed, so these two calls
    # really do evaluate val and then test.
    ...
    if cfg.include_f1:
        ...
        results["best_val_f1"] = val_metrics.get("eval_em_f1")
    print(f"[results] {cfg.dataset}/{cfg.mapping_name()} [{cfg.arm()}] "
          f"test_accuracy={results['test_accuracy']} "
          f"(best-val={results['best_val_accuracy']}) "
          f"runtime={results['train_runtime_s']}s")

    _save_train_record(cfg, internal_run, sizes, samples_per_node, results,
