"""
TAG benchmarks experiment — standalone single-run entry point.
...
(rendering each config key to the matching flag below):
    ...
    python3 -m sweep src.experiments.tag_benchmarks src/experiments/tag_benchmarks/configs/001_ablation.jsonc

Run it directly for a single config / quick iteration:

    python3 -m src.experiments.tag_benchmarks --dataset cora --max-neighbors 60
    python3 -m src.experiments.tag_benchmarks --no-magnetic --seed 1        # an ablation arm
    python3 -m src.experiments.tag_benchmarks --max-steps 4 --num-epochs 1  # smoke test
    python3 -m src.experiments.tag_benchmarks --mode data_prep --dataset pubmed --samples-per-node 2
    python3 -m src.experiments.tag_benchmarks --init my_sweep               # write a sweep template
...
{
  // ─────────────────────────────────────────────────────────────────────────
  // TAG benchmarks sweep config (JSONC: // comments + trailing commas allowed).
  // Run with:  python3 -m sweep src.experiments.tag_benchmarks <this file>
  //
  ...
  //
  // TWO-STEP WORKFLOW: run once with "mode": "data_prep" to build the cache for
  // every dataset this file references, then again with "mode": "train".
  // ─────────────────────────────────────────────────────────────────────────

  "name": "my_sweep",                 // results land in <results_dir>/<name>/
  "results_dir": "src/experiments/tag_benchmarks/results",
  ...
  "execution": {
    ...
    "sbatch": {                       // only used when mode == "sbatch"
      ...
      "partition": "frida",
      "account": "povejmo",
      "gpus": ["B200:1", "B300:1"],   // --gres=gpu:<gpus>
      "cpus": 8,
      "mem": "64G",
      "time": "24:00:00",
  ...
  // ── dataset + its cache identity as a bundle: these vary TOGETHER ─────────
  // Each entry pins the (max_neighbors, text_mapping) whose cache was built, plus
  // samples_per_node — which is NOT part of the cache's directory name, so it is
  // cross-checked against the built dataset's metadata on load.
  "dataset_arm": [
    { "dataset": "cora",   "max_neighbors": 60, "text_mapping": "target_abstract", "samples_per_node": 16 },
    { "dataset": "pubmed", "max_neighbors": 60, "text_mapping": "target_abstract", "samples_per_node": 2 }
  ],

  // ── fixed scalars ─────────────────────────────────────────────────────────
  "model_name": "meta-llama/Llama-3.2-1B",
  "impl": "v2-flex",                  // "v2-flex" | "v2-eager" (the parity/debug path)
  "dtype": "bf16",                    // "fp32" is slower but is where v0<->v2 parity is proven
  ...
  "val_subsample": 500,               // val graphs used for checkpoint selection
  "test_subsample": null,             // null = score the whole test split
...
def build_parser():
    ...
    p = argparse.ArgumentParser(
        prog="python3 -m src.experiments.tag_benchmarks",
        description="Run ONE TAG-benchmark configuration (the sweep runner invokes this per config).",
    ...
    p.add_argument("--hops", type=int, default=d.hops,
                   help="neighbourhood radius sampled per node (a data-prep knob).")
    p.add_argument("--max-neighbors", type=int, default=d.max_neighbors,
                   help="node cap per sampled neighbourhood (a data-prep knob).")
    p.add_argument("--text-mapping", choices=TEXT_MAPPINGS, default=d.text_mapping,
                   help="how much text a node contributes given its distance from the target.")
    p.add_argument("--text-mapping-param", type=float, default=d.text_mapping_param,
                   help="p for random_abstracts; character cap for truncated_text.")

    # ── model ──────────────────────────────────────────────────────────────────
    p.add_argument("--model-name", default=d.model_name)
    p.add_argument("--impl", choices=IMPLS, default=d.impl,
                   help="v2-flex suits these long sequences; v2-eager is the parity/debug path.")
    ...
    p.add_argument("--samples-per-node", type=int, default=d.samples_per_node,
                   help="neighbourhood re-samples per train node; required for data_prep, "
                        "cross-checked against the cache's metadata for train.")
    p.add_argument("--max-train-samples", type=int, default=d.max_train_samples,
                   help="subsample train nodes (omit for all).")
    p.add_argument("--max-val-samples", type=int, default=d.max_val_samples,
                   help="subsample val nodes (omit for all).")
    p.add_argument("--data-seed", type=int, default=d.data_seed,
                   help="neighbourhood-sampling RNG (data prep only).")
    p.add_argument("--use-gpu", action=B, default=d.use_gpu,
                   help="build SPD/RRWP/magnetic features on GPU (data prep).")

    # ── evaluation split sizing ────────────────────────────────────────────────
    p.add_argument("--val-subsample", type=int, default=d.val_subsample,
                   help="val graphs used for checkpoint selection (omit for the whole split).")
    p.add_argument("--test-subsample", type=int, default=d.test_subsample,
                   help="cap the final test split (omit to score all of it).")
...
def config_from_args(args):
    ...
    return RunConfig(
        mode=args.mode, dataset=args.dataset, hops=args.hops,
        max_neighbors=args.max_neighbors, text_mapping=args.text_mapping,
        text_mapping_param=args.text_mapping_param,
        ...
        max_length=args.max_length, samples_per_node=args.samples_per_node,
        max_train_samples=args.max_train_samples, max_val_samples=args.max_val_samples,
        data_seed=args.data_seed, use_gpu=args.use_gpu,
        val_subsample=args.val_subsample, test_subsample=args.test_subsample,
...
def _do_init(name):
    ...
    print(f"Wrote sweep template to {path}\n"
          f"Edit it, then run:  python3 -m sweep src.experiments.tag_benchmarks {path}")
